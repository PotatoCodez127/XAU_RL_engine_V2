...
class XAUDynamicEnv(gym.Env):
    def __init__(self, df: pd.DataFrame, initial_balance=10000.0):
        ...
        self.urgency_threshold = 240       # Start decaying reward after ~2.5 days without a trade
        ...
        self.observation_space = spaces.Box(low=-10.0, high=10.0, shape=(len(self.feature_cols) + 3,), dtype=np.float32)
    ...
    def step(self, action):
        ...
        info = {
            'balance': self.balance, 'drawdown': drawdown,
            'tp_mult_used': tp_mult_used, 'sl_mult_used': sl_mult_used
        }
