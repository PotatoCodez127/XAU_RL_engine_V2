...
class FeatureEngineer:
    ...
    def convert_to_stationary(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        ema_50h = df['env_close'].ewm(span=200, adjust=False).mean()
        ema_200h = df['env_close'].ewm(span=800, adjust=False).mean()
        
        # Normalized slope: > 0 means H4 Uptrend, < 0 means H4 Downtrend
        df['env_h4_trend'] = (ema_50h - ema_200h) / ema_200h
        
        # 2. H1 Volatility Regime (Derived via Rolling Percentiles)
        # Calculate the rolling 1-Hour (4 periods) True Range
        h1_high = df['env_high'].rolling(window=4).max()
        h1_low = df['env_low'].rolling(window=4).min()
        h1_range = h1_high - h1_low
        
        # Calculate where the current H1 range sits as a percentile of the last 24 hours (96 periods)
        df['env_h1_vol_regime'] = h1_range.rolling(window=96).rank(pct=True)
        
        # Forward fill the NaN values generated by the rolling windows
        df.fillna(method='bfill', inplace=True)
        
        # Ensure these new columns are added to the feature list for scaling later
        self.feature_cols.extend(['env_h4_trend', 'env_h1_vol_regime'])
