    ...
    if os.path.exists(STATE_FILE):
        try:
            with open(STATE_FILE, "r") as f:
                state = json.load(f)
            print(f"🔄 Recovered State: Resuming at Split {state.get('next_split')} using {state.get('resume_sac_path')}")
            return state.get("next_split", DEFAULT_START_SPLIT), state.get("resume_sac_path", DEFAULT_RESUME_SAC_PATH)
...
def save_state(next_split, resume_sac_path):
    ...
    state = {
        ...
        "resume_sac_path": resume_sac_path
    }
